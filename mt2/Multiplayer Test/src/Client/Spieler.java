...
public class Spieler
{
	...
	public Spieler(SpielLogik parent, String name, Socket socket, Bezirk[] b)
	{
		this.parent = parent;
		this.name = name;
		this.socket = socket;
		this.bezirke = b;
		this.kontostand = 160000;
		this.vorraete = new VerbrauchT();
		this.preise = Marktpreis.getMarktPreise();
	}
			...
			String nameBesitzer, int qualitaet)
	{
		...
		if (liquiditaetPruefen(kontostand, bezirke[fid / 10].getFiliale(fid % 10).getKaufPreis()) == true) {
			if (bezirke[fid / 10].getFiliale(fid % 10).getBesitzer() != null) {
				kontostand -= bezirke[fid / 10].getFiliale(fid % 10).eroeffnen(groeﬂe, typ,
	...
	{ 
		...
		double preis = preise[id].getPrice(menge, qualit‰t);
	...
	public void setMonatlicheKosten()
	{
		...
		for (Bezirk b : bezirke)
		{
			...
			{
				if (name.equals((b.getFiliale(i).getBesitzer())))
				{					
